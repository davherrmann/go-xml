...
func (cfg *Config) flatten(types map[xml.Name]xsd.Type) []xsd.Type {
	...
	flattenedTypes := map[xml.Name]xsd.Type{}
	for _, t := range types {
...
func (cfg *Config) flatten1(t xsd.Type, flattenedTypes map[xml.Name]xsd.Type, push func(xsd.Type), depth int) xsd.Type {
	if flattened, ok := flattenedTypes[xsd.XMLName(t)]; ok {
		return flattened
	}
		...
		case xsd.Builtin, *xsd.SimpleType:
			...
			if len(attributes) == 0 {
				...
				case xsd.Builtin:
					...
					return b
				case *xsd.SimpleType:
					flattened := cfg.flatten1(t.Base, flattenedTypes, push, depth+1)
					flattenedTypes[xsd.XMLName(t)] = flattened
					return flattened
				}
			}
		}
